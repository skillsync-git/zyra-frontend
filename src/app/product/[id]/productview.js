  ...
  useEffect(() => {
    ...
    const fetchProductData = async () => {
      try {
        ...
        const response = await fetch(`https://zyra-website.onrender.com/api/sarees/${id}`);
        ...
        const transformedProduct = {
          ...
          images: data.images && Array.isArray(data.images) && data.images.length > 0
            ? data.images.map(img => `https://zyra-website.onrender.com${img}`)
        ...
        if (data.category) {
          try {
            const relatedResponse = await fetch(
              `https://zyra-website.onrender.com/api/sarees?category=${encodeURIComponent(data.category)}&limit=5`
            );
  ...
  src={
    product?.images?.[0]
      ? product.images[0].startsWith('http')
        ? product.images[0]
        : `https://zyra-website.onrender.com${product.images[0]}`
      : '/default-image.jpg'
  }
