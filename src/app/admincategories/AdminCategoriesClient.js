...
export default function CategoriesAdminTable() {
  ...
  const handleDelete = async (cat) => {
    if (confirm(`Are you sure you want to delete ${cat.category_name}?`)) {
      const response = await fetch(`https://zyra-website.onrender.com/api/categories/${cat.category_id}`, {
        method: "DELETE"
      });
  ...
  const handleAddOrEdit = async e => {
    ...
    try {
      ...
      let url, method;
      if (editCategory) {
        url = `https://zyra-website.onrender.com/api/categories/${editCategory.category_id}`;
        method = "PUT";
      } else {
        url = "https://zyra-website.onrender.com/api/categories";
        method = "POST";
      }
