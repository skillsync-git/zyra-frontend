...
const fetchOrders = async () => {
  try {
    ...
    const response = await fetch("https://api-xmg2fjjbya-uc.a.run.app/api/orders/full");
    if (!response.ok) {
      ...
      if (o.items && Array.isArray(o.items) && o.items.length > 0) {
        ...
        if (firstItem.image) {
          image = firstItem.image.startsWith("http")
            ? firstItem.image
            : `https://api-xmg2fjjbya-uc.a.run.app${firstItem.image}`;
        }
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      const response = await fetch(
        `https://api-xmg2fjjbya-uc.a.run.app/api/orders/${orderId}`,
        {
          method: "PUT",
