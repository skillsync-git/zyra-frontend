...
export default function GiftCustomizationPage() {
  ...
  const fetchAllData = async () => {
    setLoading(true);
    try {
      const catRes = await fetch('https://zyra-website.onrender.com/api/categories');
      const categoriesData = catRes.ok ? await catRes.json() : [];
      
      const prodRes = await fetch('https://zyra-website.onrender.com/api/products');
      const productsData = prodRes.ok ? await prodRes.json() : [];
      
      const giftCatRes = await fetch('https://zyra-website.onrender.com/api/giftcategories');
      const giftCategoriesData = giftCatRes.ok ? await giftCatRes.json() : [];
      
      const giftItemsRes = await fetch('https://zyra-website.onrender.com/api/giftitems');
