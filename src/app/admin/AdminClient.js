  ...
  (async () => {
    try {
      const [ordersRes, statsRes] = await Promise.all([
        fetch("https://zyra-website.onrender.com/api/orders/full"),
        fetch("https://zyra-website.onrender.com/api/dashboard-stats"),
      ]);
            ...
            if (Array.isArray(o.items) && o.items.length > 0) {
              ...
              if (imgPath) {
                image = imgPath.startsWith("http")
                  ? imgPath
                  : `https://zyra-website.onrender.com${imgPath}`;
              }
            }
  ...
  const handleStatusClick = async (idx) => {
    ...
    try {
      const resp = await fetch(
        `https://zyra-website.onrender.com/api/orders/${order.order_id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
