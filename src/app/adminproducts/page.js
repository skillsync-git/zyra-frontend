...
export default function AdminProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://api-xmg2fjjbya-uc.a.run.app/api/products");
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch("https://api-xmg2fjjbya-uc.a.run.app/api/categories");
  ...
  const fetchOffers = async () => {
    try {
      const res = await fetch("https://api-xmg2fjjbya-uc.a.run.app/api/offers");
  ...
  //   try {
  //     const res = await fetch("https://api-xmg2fjjbya-uc.a.run.app/api/limitedtimesale");
  //     if (!res.ok) {
  //       console.error("Failed to fetch limited sales:", res.status);
  //       setLimitedSales([]);
  //       return;
  //     }
  //     const sales = await res.json();
  //     console.log("Fetched limited sales:", sales);
  //     // Filter only active sales (not expired)
  //     const activeSales = sales.filter(sale => {
  //       const endDate = new Date(sale.end_datetime);
  //       return endDate > new Date();
  //     });
  //     console.log("Active limited sales:", activeSales);
  //     setLimitedSales(activeSales);
  //   } catch (error) {
  //     console.error("Error fetching limited sales:", error);
  //     setLimitedSales([]);
  //   }
  // };
...
const handleSubmit = async (e) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https://api-xmg2fjjbya-uc.a.run.app/api/products/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  // const getLimitedSaleInfo = (saleId) => {
  //   if (!saleId) return null;
  //   const sale = limitedSales.find(s => s.sale_id == saleId);
  //   return sale;
  // };
          ...
          {showForm && (
            <div style={{
              ...
              >
                ...
                {/* <div style={{ marginBottom: 17 }}>
                  <label style={{ fontWeight: 500, marginBottom: 3, display: "block" }}>
                    Limited Time Sale (optional)
                    <span style={{ fontSize: 12, color: "#666", fontWeight: 400, marginLeft: 6 }}>
                      - Flash/seasonal promotions
                    </span>
                  </label>
                  <select
                    name="limited_sale_id"
                    value={form.limited_sale_id}
                    onChange={handleInput}
                    style={{
                      width: "100%",
                      padding: "9px 12px",
                  ...
                  >
                    <option value="">No Limited Sale</option>
                    {limitedSales.map((sale) => (
                      <option key={sale.sale_id} value={sale.sale_id}>
                        {sale.promotional_text} - {sale.discount_percent}% OFF (Ends: {new Date(sale.end_datetime).toLocaleDateString()})
                      </option>
                    ))}
                  </select>
                </div> */}
                    ...
                    {images.length > 0
                      ...
                      : existingImages && existingImages.map &&
                        existingImages.map((url, i) => (
                          <img
                            key={i}
                            src={`https://api-xmg2fjjbya-uc.a.run.app${url}`}
                            alt="img"
                            width={34}
                            height={34}
